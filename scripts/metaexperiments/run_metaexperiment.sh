...
REMOTE_HOST="root@194.68.245.42"
REMOTE_PORT="22112"
...
generate_layer_sizes() {
    ...
    for size in "${sizes[@]}"; do
        ...
        if [ "$prefix" = "" ]; then
            new_prefix="784,$size"  # First layer is always 784 (MNIST input)
        fi
        generate_layer_sizes $((n_layers-1)) "$2" "$new_prefix"
    done
}
...
for n_layer in "${N_LAYERS[@]}"; do
    ...
    while IFS= read -r layer_sizes; do
        ...
        while IFS= read -r signific_connections; do
            for pathway in "${PATHWAY_INTERACTION[@]}"; do
                ...
                echo "bash scripts/experiments/run_experiment.sh \"$REMOTE_HOST\" \"$REMOTE_PORT\" \"$SSH_KEY\" \"$REMOTE_DIR\" \"$LOCAL_DIR\" \"scripts/experiments/mnist.py\" --layer_sizes=$layer_sizes --hidden_signific_connections=$signific_connections --allow_pathway_interaction=$pathway --n_epochs=20" >> $JOB_FILE
                chmod +x $JOB_FILE
    ...
    done < <(generate_layer_sizes $n_layer "${LAYER_SIZES[*]}" "")
done
